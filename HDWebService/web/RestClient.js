...
// Model for Service entity
    models.Service = Backbone.Model.extend({
        urlRoot: "http://localhost:51546/HDWebService/webresources/entity.service/",
        idAttribute: 'serviceno',
        defaults: {
            thumbnail: "",
            description: "",
            servicename: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('serviceno');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:51546/HDWebService/webresources/entity.service/';
            }
    ...
    // Collection class for Service entities
    models.ServiceCollection = Backbone.Collection.extend({
        model: models.Service,
        url: "http://localhost:51546/HDWebService/webresources/entity.service/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ServiceCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-service-list-itemis template identifier for item
                        templateName: '#tpl-service-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.service = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.service,
                    // tpl-service-details is template identifier for chosen model element
                    templateName: '#tpl-service-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Service();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-service-details is a template identifier for chosen model element
                templateName: '#tpl-service-details',
